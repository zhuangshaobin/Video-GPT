...
from LVM import LVMTraining, LVMProcessor
if torch_npu is not None:
    from LVM.transform.fa_transform import replace_attention, replace_simple_attention
else:
    from LVM.transform.sdpa_transform import replace_attention as replace_simple_attention
from LVM.acceleration.parallel_states import initialize_sequence_parallel_state, hccl_info
from LVM.train_helper import DatasetFromVideo, TrainDataCollator
...
def main(args):
    ...
    model = LVMTraining.from_pretrained(args.model_name_or_path, load_llm_ckpt=False)
    ...
    model.llm = replace_simple_attention(model.llm)
    ...
    dataset = DatasetFromVideo(video_dir_path=args.video_dir_path,
                               processer=processor,
    ...
    collate_fn = TrainDataCollator(pad_token_id=processor.text_tokenizer.eos_token_id, 
        ...
        for data in loader:
            ...
            with accelerator.accumulate(model):
                with torch.no_grad():
                    ...
                    output_images = vae_encode(vae, data["output_images"], weight_dtype)

                model_kwargs = dict(input_ids=data['input_ids'], 
                                    input_img_latents=input_pixel_values, 
                                    input_image_sizes=data['input_image_sizes'], 
                                    attention_mask=data['attention_mask'], 
                                    position_ids=data['position_ids'], 
                                    denoise_image_sizes=data["denoise_image_sizes"],
                                    time_emb_inx=data["time_emb_inx"],
