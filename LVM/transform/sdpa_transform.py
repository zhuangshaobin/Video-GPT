...
def new_forward(
        ...
        causal_mask = attention_mask.to(dtype=query_states.dtype)
        ...
        query_states = query_states.transpose(1, 2).contiguous()  # B S N D
        key_states = key_states.transpose(1, 2).contiguous()  # B S N D
        value_states = value_states.transpose(1, 2).contiguous()  # B S N D
...
def new_attn_forward(self, query: Tensor, key: Tensor, value: Tensor, batch_dim_idx: int, *args: Any, **kwargs) -> Tensor:
    ...
    #out shape : e.g., [s:h/p:]
    query_layer = query_layer.transpose(1, 2).contiguous()  # B N S D
    key_layer = key_layer.transpose(1, 2).contiguous()  # B N S D
    value_layer = value_layer.transpose(1, 2).contiguous()  # B N S D
...
def replace_attention(model):
    for module in model.modules():
        if isinstance(module, Phi3SdpaAttention):  # 检查层的类型是否为lora.Linear
            module.forward = MethodType(new_forward, module)
            module.local_attn = torch.nn.functional.scaled_dot_product_attention
